...
from .serializers import TicketSerializer, Ai_SummarrySerializer, AssignmentSerializer, WorklogSerializer
...
class TicketViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_staff:
            return queryset
        return queryset.filter(user=user).defer('ai_summarry', 'ticket_worlogs')
    ...
    def perform_update(self, serializer):
        if self.request.user != self.get_object().user and not self.request.user.is_staff:
...
class AiSummarry(ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        serializer = Ai_SummarrySerializer(data=request.data)
